<%@page import="uva.model.Fornecedor"%>
...
<body class="container-fluid row text-light">
	<!-- JSP -->
<%
	Fornecedor f = (Fornecedor) request.getAttribute("dados");
	String acao="incluir-fornecedor";
	String id="0",botao="",cnpj="", nome="", telefone="", email="";
	if (f!=null){
		acao		="alterar-fornecedor";
		id			= String.format("%d", f.id);
		cnpj 		= String.format("value='%s'",f.cnpj);
		nome 		= String.format("value='%s'",f.nome);
		telefone 	= String.format("value='%s'",f.telefone);
		email 		= String.format("value='%s'",f.email);
		botao		= "value='Alterar'";
	}	
%>	
<!-- FIM-JSP -->
	...
	<main class="col-lg-8 col-md-12 col-sm-12 col-xs-12"
		...
		<header>
			<h1 class="text-center">Dados dos Fornecedores</h1>
		</header>
		...
		<form action="LojaFC" method="post" class="form">
			...
			<div class="row mb-2">
				<div class="col">
					<input type="text" class="form-control m-0" placeholder="CNPJ" name="cnpj" <%=cnpj%>>
				</div>
				<div class="col">
